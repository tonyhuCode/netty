...
public class HttpServerUpgradeHandler extends HttpObjectAggregator {
    ...
    private boolean upgrade(final ChannelHandlerContext ctx, final FullHttpRequest request) {
        ...
        final List<String> requestedProtocols = splitHeader(request.headers().get(HttpHeaderNames.UPGRADE));
        UpgradeCodec upgradeCodec = null;
        String upgradeProtocol = null;
        for (String p: requestedProtocols) {
            UpgradeCodec c = upgradeCodecFactory.newUpgradeCodec(p);
            if (c != null) {
                upgradeProtocol = p;
                upgradeCodec = c;
                break;
        ...
        final FullHttpResponse upgradeResponse = createUpgradeResponse(upgradeProtocol, upgradeCodec);
     ...
     */
    private static FullHttpResponse createUpgradeResponse(String upgradeProtocol, UpgradeCodec upgradeCodec) {
     ...
     */
    private static List<String> splitHeader(CharSequence header) {
        final StringBuilder builder = new StringBuilder(header.length());
        final List<String> protocols = new ArrayList<String>(4);
